# PERSONACLICK JS SDK v3 Common Mistakes

## Using a non-PERSONACLICK global in PERSONACLICK integrations
...
Use PERSONACLICK/personaclick naming consistently unless the client explicitly uses a white-label global.
...
```js
window.personaclick('init', SHOP_TOKEN, 'web-production-v2-long-name')
window.personaclick('init', SHOP_TOKEN, 'web-prod!')
```

Good:

```js
window.personaclick('init', SHOP_TOKEN, 'web_prod')
```
...
```js
window.personaclick('track', 'view', product.id)
```
...
```js
window.personaclick('init', token)
```
...
```js
window.personaclick.trackPurchase(order)
window.personaclick.purchase(order)
```

Good:

```js
window.personaclick('track', 'purchase', orderData)
```
...
```js
window.personaclick('track', 'purchase', { order: order.id, products: [] })
```
...
```js
window.personaclick('track', 'lead', { value: '12.5' })
```

Good:

```js
window.personaclick('track', 'lead', { value: 12 })
```
...
```js
window.personaclick('track', 'cart', [{ id: '1', amount: 2 }]) // full cart snapshot
window.personaclick('track', 'cart', { id: '1', amount: 2 }) // single item event
```
...
```js
window.personaclick('cart', 'clear', { email: customer.email })
```
...
```js
window.personaclick('recommend', 'dynamic', params, success, failure)
```

Prefer dynamic block code:

```js
window.personaclick('recommend', 'block-code', { limit: 8 }, success, failure)
```
...
```js
window.personaclick('recommend', 'block-code', { with_locations: true }, success, failure)
```

Good:

```js
window.personaclick('recommend', 'block-code', {
  extended: true,
  with_locations: true,
}, success, failure)
```
...
- Product/order IDs match the merchant catalog IDs known to PERSONACLICK.
...
- No imports from `personaclick-js-sdk-v3/src/...` in client projects.
- No undocumented `window.personaclick.someMethod()` usage.
