---
name: personaclick-js-sdk-v3
description: Helps client agents integrate, debug, or review PERSONACLICK JS SDK v3 usage in browser websites and SPAs. Use when adding PERSONACLICK tracking, recommendations, search, subscriptions, profile, cart, products, loyalty, NPS, or other personaclick queue commands.
---

# PERSONACLICK JS SDK v3

You are helping a client integrate or maintain PERSONACLICK JS SDK v3.

The SDK is a browser queue API. The default global is `window.personaclick`; white-label builds may expose another global. Use `personaclick`, PERSONACLICK domains, and PERSONACLICK dashboard terminology consistently. Do not import SDK internals from `src/` in client projects unless explicitly working inside the SDK repository.

## Mandatory workflow

1. Inspect the target project first: framework, entrypoints, existing PERSONACLICK snippets, router/SPA behavior, consent handling, ecommerce data model.
2. Find whether `window.personaclick` or a white-label global already exists. Avoid duplicate bootstraps and duplicate `init` calls.
...
9. Treat documentation for S2S, mobile SDKs, notification center, full order history, catalog imports, exports, and force profile changes as server/backend guidance unless `references/api.md` explicitly shows a browser `personaclick` command.
10. Add verification steps: network request check, callback check, console errors, and duplicate event detection.

## Loading pattern

Use the queue stub before the SDK bundle is available. The default bundle CDN is `cdn.personaclick.com`; use another CDN only when the client explicitly provides one:
...
<script async src="https://cdn.personaclick.com/v3.js"></script>
```

For SPA mode:

```js
window.personaclick('init', 'SHOP_TOKEN', 'web', undefined, undefined, undefined, true)
```
...
- `references/examples.md` — common integration examples adapted to PERSONACLICK/personaclick naming.
