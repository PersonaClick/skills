...
AI-agent skills for working with PERSONACLICK SDKs.
...
- `js-sdk/` - integration, review, and debugging guidance for PERSONACLICK JS SDK v3.
...
```bash
npx skills add personaclick/skills --skill '*' -g
```
...
```bash
# Codex
npx skills add personaclick/skills --skill '*' -g -a codex

# Claude Code
npx skills add personaclick/skills --skill '*' -g -a claude-code

# Cursor
npx skills add personaclick/skills --skill '*' -g -a cursor

# Windsurf
npx skills add personaclick/skills --skill '*' -g -a windsurf
```

For a non-interactive install, add `-y`:

```bash
npx skills add personaclick/skills --skill '*' -g -a codex -y
```

To list skills before installing:

```bash
npx skills add personaclick/skills --list
```
...
```bash
npx skills update personaclick-js-sdk-v3 -g
```
...
```bash
git clone https://github.com/personaclick/skills.git
cd skills
```
...
```bash
git clone git@github.com:personaclick/skills.git
cd skills
```
...
Install all PERSONACLICK skills:
...
If your Claude Code version does not load `SKILL.md` directories directly, add the PERSONACLICK skill to your project instructions instead. From your project directory:
...
When working with PERSONACLICK JS SDK v3, read and follow:
...
3. Add a project rule that points the agent to the PERSONACLICK skill files.

Example project rule:

```text
When adding, reviewing, or debugging PERSONACLICK JS SDK v3 code, follow these local instructions:
...
Do not invent PERSONACLICK SDK API calls.
```
...
After installation, ask your agent for a concrete PERSONACLICK SDK task. Good prompts mention PERSONACLICK, JS SDK v3, `personaclick`, or the integration area you want to work on.

### Add PERSONACLICK SDK To A Project

```text
Add PERSONACLICK JS SDK v3 to this storefront. Initialize it once, make it safe for SSR, and show where SHOP_TOKEN should come from.
```

### Review An Existing Integration

```text
Review the existing PERSONACLICK JS SDK integration in this project. Check duplicate init calls, SPA routing, cart and purchase tracking, and undocumented personaclick commands.
```

### Debug Tracking

```text
Find why PERSONACLICK product view and cart events are not sent correctly. Inspect the current personaclick calls and give me a verification checklist for the browser.
```

### Add Recommendations

```text
Add PERSONACLICK recommendations to the product page. Use product-related recommendations, preserve recommendation attribution for add-to-cart events, and avoid SDK calls that are not documented.
```

### Check Security Boundaries

```text
Check this PERSONACLICK integration for browser-side secrets or server-only API usage. Move anything unsafe to backend guidance instead of exposing it in client code.
```
...
```text
Use the PERSONACLICK JS SDK skill from /absolute/path/to/skills/js-sdk.
Before changing code, read SKILL.md and the relevant files in references/.
Use only documented personaclick commands and include manual verification steps.
```
...
Ask your agent to work on a task that clearly requires the PERSONACLICK SDK, for example:

```text
Check the PERSONACLICK JS SDK integration in this project and find initialization mistakes.
```

Expected behavior:

- the agent says it is using the PERSONACLICK JS SDK skill;
